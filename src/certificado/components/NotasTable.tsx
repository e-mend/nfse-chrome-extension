...
// ─── Downloads ──────────────────────────────────────────────────────────────
// chrome.downloads is available in extension pages with the "downloads"
// permission (declared in manifest.config.ts). Fall back to a synthetic anchor
// click for `vite preview` / non-extension contexts (e.g. when devs open
// `dist/` over the file:// scheme).
...
function downloadBlob(blob: Blob, filename: string) {
  ...
  const dl = (globalThis as { chrome?: typeof chrome }).chrome?.downloads;
  if (dl?.download) {
    dl.download({ url, filename, saveAs: false }, () => {
      // Revoke after the download has been queued — too early and chrome
      // can't fetch the blob: URL anymore.
 ...
 * (parser + pdf-lib layout per NT-008 v1.0) plus the same `chrome.downloads`
 * fallback used for XML downloads.
 *
...
async function downloadPdf(nota: NotaDoc, pattern?: string | null): Promise<Uint8Array> {
  ...
  // pdf-lib returns a Uint8Array; wrap it in a Blob so chrome.downloads can
  // pick it up. The BlobPart conversion sidesteps a SharedArrayBuffer issue
  // some pdf-lib builds emit on Chrome 121+ (`Uint8Array<ArrayBufferLike>`).
